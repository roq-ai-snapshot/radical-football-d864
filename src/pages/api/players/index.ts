...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createPlayer() {
    ...
    const body = { ...req.body };
    if (body?.parent_access_request?.length > 0) {
      const create_parent_access_request = body.parent_access_request;
      body.parent_access_request = {
        create: create_parent_access_request,
      };
    } else {
      delete body.parent_access_request;
    }
